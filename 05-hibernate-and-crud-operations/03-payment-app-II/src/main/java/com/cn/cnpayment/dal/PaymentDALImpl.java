...
public class PaymentDALImpl implements PaymentDAL{
	...
	public void update(Payment updatePayment) {
		Session session = entityManager.unwrap(Session.class);
		//fetching the curr item
		Payment currPayment = session.get(Payment.class, updatePayment.getId());
		currPayment.setPaymentType(updatePayment.getPaymentType());
		currPayment.setDescription(updatePayment.getDescription());
		//update db
		session.update(currPayment);
	}

	@Override
	public void updateDescription(int paymentId, String description) {
		Session session = entityManager.unwrap(Session.class);
		Payment currPayment = session.get(Payment.class, paymentId);
		currPayment.setDescription(description);
		//update db
		session.update(currPayment);
	}
